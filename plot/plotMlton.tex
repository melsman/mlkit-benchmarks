...
\def\twinplot#1#2{
  \pgfplotsforeachungrouped \row in {0,...,18}{
    \pgfplotsforeachungrouped \col in {2,...,3}{
      \edef\justplotit{
        \noexpand\addplot+[bar shift=(\col-2.5)*\pgfplotbarwidth]
...
\def\triplot#1#2{
  \pgfplotsforeachungrouped \row in {0,...,18}{
    \pgfplotsforeachungrouped \col in {1,...,3}{
      \edef\justplotit{
...
\begin{groupplot}
  ...
  \nextgroupplot [legend to name={CommonLegend},legend style={legend columns=4}] \twinplot{a}{DataMlton.csv}

  \addlegendentry{RI \rule{5mm}{0pt}}
  \addlegendentry{GC+RI}
%  \addlegendentry{GENGC}

  %\addplot[red,sharp plot,update limits=true] coordinates {(kkb,1) (ratio,1)};
...
\end{groupplot}
  ...
  \node[at={(15,3.5)}]{\ref{CommonLegend}};
